...
public class SwapConditionalFairValueProcess<T extends ZCBond_ProductConditionalFairValue_ModelInterface> extends AbstractProductConditionalFairValueProcess<T>{
	 ...
	 * fair value zero at time zero is calculated and set. This calculation is based on the underlying model.
	 * 
	 * @param underlyingModel The underlying model with respect to which the fair value of the coupon bond is evaluated.
	 * @param paymentDatesFixingDates This array contains the fixing dates and the payment dates. It is assumed that the first index is the first fixing date and is strictly greater than zero. The last index is the last payment date. All other indices are payment date of the previous period and fixing date of the next period at the same time. 
...
//	}
	 ...
	 * of the coupon payments at time index nextPaymentDateIndex or later 
	 * is calculated. 
	 * 
	...
	//////
	////
	// Multi Curve Test Versions (Deterministic Discounting)
	////
	//////
	
	// TODO: Change name if advantageous, write test.
	// Proofreading Notes: Master Thesis, Block 3, p.13)
	/**
	 * This method returns the fair value of a swap with respect to the defaultable Libor L^d evaluated at the time with index timeIndex.
	 * ( In formulas: sum_{i= k_timeIndex}^{n-1} ( L^d(T_i, T_i+1; t_timeIndex) - C ) (T_{i+1} - T_i) )
	 * Where k_timeIndex is the smallest i such that t_timeIndex <= T_i and C is the coupon payment.
	 * 
	 * @param timeIndex The index of the time at which the product is evaluated.
	...
	public RandomVariableInterface getFairValue(int timeIndex) throws CalculationException {
		...
		if(nextDateIndex + 1 != numberOfDates) {
			...
			// The discounting adjustment (multi-curve) discountingAdjustment(evaluationTime, T_{indexFloatingLegFixingDate}) := exp( int_evaluationTime^{T_{indexFloatingLegFixingDate}} lambda(s) ds ) 
 			double currentDiscountingAdjustmentEvalFixing = 1.0;
 			
 			// The discounting adjustment (multicurve) discountingAdjustment(T_{indexFloatingLegFixingDate}, T_{indexFloatingLegFixingDate + 1}).
 			double currentDiscountingAdjustmentFixingPayment = 1.0;
			...
			for(int indexFloatingLegFixingDate = nextDateIndex; indexFloatingLegFixingDate < numberOfDates - 1; indexFloatingLegFixingDate++ ) {
				...
				// The current discounting adjustment from evaluation time to fixing date is assigned.
	 			currentDiscountingAdjustmentEvalFixing = this.underlyingModel.getDiscountingAdjustment(evaluationTime, paymentDatesFixingDates[ indexFloatingLegFixingDate ]);
				
	 			// The current discounting adjustment from fixing date to payment date is assigned. 
	 			currentDiscountingAdjustmentFixingPayment = this.underlyingModel.getDiscountingAdjustment( paymentDatesFixingDates[ indexFloatingLegFixingDate ], paymentDatesFixingDates[ indexFloatingLegFixingDate + 1 ] );
	 			...
	 			// = [ alpha(T_i, T_{i+1}) * P^d(T_i;t) - P^d(T_{i+1}; t) / alpha(T_i,T_{i+1}) ]  / alpha(t,T_{i}) (Master Thesis, Block 3, p.4).
	 			// Where i = indexFloatingLegFixingDate and t = evaluationTime and alpha(a,b) is the discountingAdjustment from a to b.
	 			currentPaymentFairValue = ( currentBondEvaluationTimeFixingDate.mult( currentDiscountingAdjustmentFixingPayment ).addProduct(currentBondEvaluationTimePaymentDate, -1.0/currentDiscountingAdjustmentFixingPayment) ).mult( 1.0 / currentDiscountingAdjustmentEvalFixing);
	 ...
	 * 
	 * The fair value at evaluation time ( the time with index timeIndex ) 
	 * of the non-defaultable coupon payments at time index nextPaymentDateIndex and following 
	 * is calculated. 
	 * 
	 * @param timeIndex The index of the time at which the future coupon payments are evaluated.
	 * @param nextPaymentDateIndex The index of the time at which the next coupon is payed.
	...
	private double parSwapRateAtZeroCalculation() throws CalculationException {
		...
		// TODO: Support non-defaultable Coupon-Bond.
		...
		// The fair value of the fixed leg is .
		...
		////
		// Subtracting the fair value of the fixed payments from the fair value of the floating payments. 
		// This is par swap value. Next the par swap rate is assigned to the instance variable swap rate.
		////
